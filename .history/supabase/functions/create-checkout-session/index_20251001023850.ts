...
// Si defines TRIAL_PERIOD_DAYS en supabase/.env, se aplica como trial en la suscripción
const TRIAL_DAYS = Number(Deno.env.get("TRIAL_PERIOD_DAYS") ?? "0") || 0;

// Si quieres bypass total para smoke tests, pon BYPASS_CHECKOUT=1 en supabase/.env
...
Deno.serve(async (req) => {
  ...
  // Bypass opcional controlado por env (útil para aislar problemas del runtime/JWT)
  if (BYPASS_CHECKOUT) {
  ...
  // --- DEBUG: verifica que la clave y el price coinciden ---
  const keyUsedPrefix = (Deno.env.get("STRIPE_API_KEY") ?? "").slice(0, 12);
  console.log("[DEBUG] STRIPE_API_KEY (prefix):", keyUsedPrefix);

  try {
    const probe = await stripe.prices.retrieve(String(priceId));
    console.log(
      "[DEBUG] Price OK:",
      probe.id,
      "currency:",
    ...
    console.error("[DEBUG] retrieve(price) FAILED with this key:", e);
    return json(
      {
        error:
          "Stripe key / price mismatch. Asegúrate de usar STRIPE_API_KEY de TEST de la misma cuenta/mode que creó ese price_id.",
      },
      400
    );
  }
  ...
  try {
    ...
    return json(
      { id: session.id, url: session.url, checkoutUrl: session.url },
      200
    );
  } catch (err: unknown) {
