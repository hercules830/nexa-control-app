...
function DashboardPage({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchInsumos = async () => {
      console.log("Intentando cargar SOLO los insumos...");
      try {
        ...
        console.log("Insumos cargados exitosamente:", data);
        setInsumos(data);
        toast.success("¡Datos de insumos cargados!");

      } catch (error) {
        console.error("ERROR DETALLADO AL CARGAR INSUMOS:", error);
  ...
  const handleLogout = async () => {
    try {
      ...
      if (error) throw error;
      // No necesitamos hacer nada más aquí, el listener en App.jsx se encargará.
    } catch (error) {
  ...
  const openEditModal = (item, type) => {
    ...
    // REEMPLAZA tu useMemo completo por este
    const reportData = useMemo(() => {
      ...
      return {
        ...
        filteredSales // Devolvemos las ventas filtradas para usarlas fuera
      };
    }, [salesHistory, insumos, selectedDate]); // Mantenemos el array de dependencias correcto
  
    // --- NUEVA LÓGICA DE GRÁFICO (FUERA DE useMemo) ---
    const salesByProductId = (reportData.filteredSales || []).reduce((acc, sale) => {
            ...
            <section className={styles.card}>
              ...
              <div className={styles.chartContainer}>
                {chartLabels && chartLabels.length > 0 ? (
                  <SalesChart labels={chartLabels} dataValues={chartDataValues} />
                ) : (
