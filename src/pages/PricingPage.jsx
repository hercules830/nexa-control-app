...
function PricingPage() {
  ...
  const ensureStripeCustomer = async (currentUser, accessToken) => {
    ...
    try {
      ...
      toast.error(
        err?.message || "No se pudo crear tu perfil de cliente.",
        { id: toastId }
      );
  ...
  const handleCheckout = async (priceId) => {
    ...
    try {
      ...
      // *** CAMBIO CLAVE ***
      // Usa fetch en lugar de supabase.functions.invoke para ver cuerpo de error
      ...
      if (!res.ok) {
        // Mostramos el error real devuelto por la función
        let msg = `HTTP ${res.status}`;
        try {
          const j = JSON.parse(text);
          if (j?.error) msg = j.error;
        } catch {}
        console.error("[pricing] create-checkout-session error:", text);
        toast.error(msg);
        return;
      }
