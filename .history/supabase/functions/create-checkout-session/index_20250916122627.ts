...
// REEMPLAZA ESTE ARCHIVO COMPLETO CON LA NUEVA LÓGICA
...
serve(async (req) => {
  ...
  try {
    ...
    // --- NUEVA LÓGICA ---
    // 1. Intenta obtener el perfil del usuario.
    ...
    // 2. Si el perfil NO existe, créalo.
    if (!profile) {
      const { data: newUserProfile, error: insertError } = await supabaseClient
        .from('profiles')
        .insert({ id: user.id, subscription_status: 'inactive' })
        .select('stripe_customer_id')
        .single();
