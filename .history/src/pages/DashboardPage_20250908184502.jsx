...
function DashboardPage({ user }) {
  ...
  const reportData = useMemo(() => {
    // 1. Agrupamos las ventas por ticketId de forma más robusta
    const groupedSales = salesHistory.reduce((acc, sale) => {
      const ticketId = sale.ticketId;
      if (!acc[ticketId]) {
        acc[ticketId] = {
          // Guardamos el ticketId aquí para un acceso fácil y seguro
          ticketId: ticketId,
          items: [],
    ...
    // 2. Convertimos el objeto en un array y lo ordenamos de forma segura
    const groupedSalesArray = Object.values(groupedSales).sort(
      (a, b) => b.ticketId - a.ticketId // Ahora ordenamos usando el ticketId guardado
    );

    // El resto de los cálculos se quedan exactamente igual
