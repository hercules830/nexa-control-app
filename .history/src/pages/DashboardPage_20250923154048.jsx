...
function DashboardPage({ user, subscriptionStatus, refreshProfile }) {
  ...
  const reportData = useMemo(() => {
    ...
    const groupedSalesArray = Object.values(groupedSales).sort((a, b) => b.ticket_id - a.ticket_id);
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <section className={styles.card}>
                  ...
                  <form onSubmit={handleReponerStock} className={styles.insumoFormGrid}>
                    ...
                    <input type="number" placeholder="Cantidad Comprada" value={reponerCantidad} onChange={(e) => setReponerCantidad(e.target.value)} className={styles.input} min="0"/>
                    <input type="number" placeholder="Costo Total de la Compra ($)" value={reponerCostoTotal} onChange={(e) => setReponerCostoTotal(e.target.value)} className={styles.input} min="0" step="0.01"/>
                  ...
                  <ul className={styles.ticketGroupList}>
                    {reportData.groupedSalesArray.length === 0 ? (<p>Aún no se han registrado ventas.</p>) : (
                      reportData.groupedSalesArray.map((ticket) => (
                        <li key={ticket.ticket_id} className={styles.ticketGroup}>
  ...
  <main className={styles.tabContent}>
    ...
    <div className={styles.subscriptionSection}>
      ...
      {/* Condición 1: El plan está activo o en prueba */}
