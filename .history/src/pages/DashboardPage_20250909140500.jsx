...
function DashboardPage({ user }) {
  ...
  // Por ahora, el modal es solo para productos, no para insumos.
          ...
          <section className={styles.card}>
            ...
            <form onSubmit={handleAddProduct} className={styles.productFormGrid}>
              ...
              {newProduct.type === 'direct' ? (
                <select value={newProduct.insumoId} onChange={(e) => {const selected = insumos.find(i => i.id === e.target.value); setNewProduct({...newProduct, insumoId: e.target.value, name: selected ? selected.name : ""});}} className={styles.input}>
                    ...
                    {filteredInsumos.length === 0 ? (<p>No se encontraron insumos.</p>) : (filteredInsumos.map((insumo) => (
                      <li key={insumo.id} className={`${styles.insumoItem} ${(insumo.alertThreshold != null && insumo.quantity <= insumo.alertThreshold) ? styles.lowStock : ""}`}>
                        <div className={styles.insumoInfo}>
                          ...
                          <span className={styles.insumoCost}>Costo: ${insumo.cost.toFixed(2)} / {insumo.unit}</span>
                        </div>
                  ...
                  <li key={ticket.ticketId} className={styles.ticketGroup}>
                    <div className={styles.ticketHeader}>
                      <div className={styles.ticketDetails}>
                        ...
                        <span className={`${styles.paymentBadge} ${styles[ticket.method]}`}>{ticket.paymentMethod}</span>
                      </div>
