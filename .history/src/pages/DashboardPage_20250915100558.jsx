...
function DashboardPage({ user }) {
  ...
  // --- CORRECCIÓN 2: Lógica para mostrar el mensaje de bienvenida solo una vez ---
  useEffect(() => {
    // Revisa si ya se mostró el mensaje en la sesión actual del navegador
    const welcomeMessageShown = sessionStorage.getItem('welcomeMessageShown');
    
    // Si hay un usuario y el mensaje no se ha mostrado, muéstralo y guarda una bandera.
  ...
  // --- CORRECCIÓN 1: Lógica de carga de datos y suscripción a cambios en tiempo real ---
  useEffect(() => {
    if (!user) return;
    ...
    const channel = supabase.channel('public-db-changes')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload) => {
        ...
        // En lugar de recargar todo, solo actualiza lo que cambió
        if (payload.table === 'products') {
          fetchProducts();
        } else if (payload.table === 'insumos') {
          fetchInsumos(); // <- Esto asegura que los insumos se actualicen
    ...
    // Limpieza: remover el canal al desmontar el componente
    return () => {
      supabase.removeChannel(channel);
    };
  }, [user]); // Se ejecuta solo cuando el usuario cambia
  ...
  const handleLogout = async () => {
    try {
      ...
      if (error) throw error;
      // --- CORRECCIÓN 2: Limpia la bandera al cerrar sesión ---
