...
function DashboardPage({ user }) {
  ...
  const [newInsumo, setNewInsumo] = useState({
    name: "",
    quantity: "",       // Esto ahora será la cantidad de UNIDADES en el paquete
    unit: "",
    totalCost: "",      // Nuevo campo para el costo TOTAL del paquete
    alertThreshold: "",
  });
  ...
  const handleAddProduct = async (event) => {
    ...
    if (newProduct.type === 'recipe') {
      ...
      finalName = insumoSource.name; // Usamos el nombre del insumo para consistencia
    }
  ...
  const handleAddInsumo = async (event) => {
    ...
    if (
      ...
      toast.error("Nombre, cantidad, unidad y costo total son obligatorios.");
      return;
    }

    // --- ¡AQUÍ ESTÁ EL CÁLCULO AUTOMÁTICO! ---
    ...
    try {
      ...
      await addDoc(insumosCollectionRef, {
        ...
        quantity: quantity,
        unit: newInsumo.unit,
        cost: costPerUnit, // Guardamos el costo por unidad calculado
      ...
      setNewInsumo({ name: "", quantity: "", unit: "", cost: "", totalCost: "", alertThreshold: "" });
    } catch (error) {
  ...
  const handleUpdateInsumo = async (event) => {
    ...
    try {
      ...
      await updateDoc(insumoDocRef, {
        ...
        // No editamos la cantidad aquí, eso se hace con +/-
  ...
  return (
    ...
    >
              ...
              <form onSubmit={handleAddProduct} className={styles.productFormGrid}>
                {newProduct.type === 'direct' ? (
                  <select
                    ...
                    onChange={(e) => {
                      const selectedInsumo = insumos.find(i => i.id === e.target.value);
                      setNewProduct({
                        ...newProduct,
                ...
                <div className={styles.productTypeSelector}>
                  <label>
                    <input type="radio" value="direct" checked={newProduct.type === 'direct'} onChange={(e) => setNewProduct({ ...newProduct, type: e.target.value, recipe: [] })}/> 
                    Reventa (Usa 1 Insumo)
                  </label>
                  <label>
                    <input type="radio" value="recipe" checked={newProduct.type === 'recipe'} onChange={(e) => setNewProduct({ ...newProduct, type: e.target.value, insumoId: '' })}/> 
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
            <section className={styles.card}>
              ...
              <form onSubmit={handleAddInsumo} className={styles.insumoFormGrid}>
                ...
                <input
                  type="number"
                  placeholder="Costo Total del Paquete ($)"
                  className={styles.input}
                  value={newInsumo.totalCost}
                  onChange={(e) => setNewInsumo({ ...newInsumo, totalCost: e.target.value })}
                  min="0"
                />
                <input
                  type="number"
      ...
      Costo: ${insumo.cost.toFixed(2)} / {insumo.unit}
    </span>
