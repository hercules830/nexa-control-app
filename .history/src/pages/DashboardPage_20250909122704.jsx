...
function DashboardPage({ user }) {
  ...
  const handleAddProduct = async (event) => {
    ...
    if (newProduct.recipe.length === 0) {
        toast.error("La receta no puede estar vacía. Agrega al menos un insumo.");
        return;
    }
    const calculatedCost = newProduct.recipe.reduce((sum, item) => sum + (item.quantityUsed * item.cost), 0);
    try {
      const productsCollectionRef = collection(db, "users", user.uid, "products");
      ...
      toast.success("¡Producto y receta guardados!");
      setNewProduct({ name: "", price: "", recipe: [] });
    } catch (error) {
