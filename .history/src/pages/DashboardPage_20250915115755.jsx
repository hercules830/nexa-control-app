...
function DashboardPage({ user }) {
  ...
  // --- CORRECCIÓN 2: Funciones de carga de datos movidas aquí para poder reutilizarlas ---
  ...
  const handleAddProduct = async (event) => {
    ...
    if (newProduct.type === 'recipe') {
      ...
      // --- CORRECCIÓN 1 (para un error futuro, igual al de insumos) ---
    ...
    try {
      ...
      // --- CORRECCIÓN 2: Forzamos la actualización de la lista de productos ---
      fetchProducts();
  ...
  const handleDeleteProduct = async (productId, productName) => {
    if (window.confirm(`¿Seguro que quieres eliminar el producto "${productName}"?`)) {
      try {
        ...
        // --- CORRECCIÓN 2: Forzamos la actualización también al borrar ---
        fetchProducts();
      } catch (error) {
  ...
  const handleUpdateProduct = async (event) => {
    ...
    try {
      ...
      toast.success("Producto actualizado con éxito.");
       // --- CORRECCIÓN 2: Forzamos la actualización también al editar ---
      fetchProducts();
  ...
  const handleAddInsumo = async (event) => {
    ...
    try {
      ...
      // --- CORRECCIÓN 2: Forzamos la actualización de insumos ---
      fetchInsumos();
  ...
  const handleAddItemToTicket = (event) => {
    ...
    // --- CORRECCIÓN 1: Convertir el ID a número ---
  ...
  const handleFinalizeSale = async () => {
    ...
    try {
      ...
      // --- CORRECCIÓN 2: Forzamos la actualización de ventas e insumos ---
      fetchSales();
      fetchInsumos();
      setCurrentTicket([]);
