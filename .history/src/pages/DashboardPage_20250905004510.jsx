...
function DashboardPage({ user }) {
  ...
  const handleAddItemToTicket = (event) => {
    ...
    if (productToAdd.quantity < saleQuantity) {
      toast.error(`Stock insuficiente. Solo quedan ${productToAdd.quantity} unidades.`);
      return;
    }
  ...
  const reportData = useMemo(() => {
    ...
    const totalSalesCount = new Set(salesHistory.map(sale => sale.ticketId)).size;
                ...
                {salesHistory.length === 0 ? (
                  ...
                  salesHistory.map((sale) => (
                    <li key={sale.id} className={styles.salesItem}>
                      <span>
                        <strong>{sale.productName}</strong> - {sale.quantity} unidad(es)
                      </span>
                      <div className={styles.saleDetails}>
                        <span className={styles.profitText}>
                          +${sale.profit.toFixed(2)} Ganancia
                        </span>
                        <span className={styles.saleDate}>{new Date(sale.date).toLocaleString('es-MX')}</span>
                      </div>
                    </li>
                  ))
                )}
