...
function DashboardPage({ user }) {
  ...
  const handleAddRecipeItemToEdit = () => {
    ...
    // Actualizamos el estado del item que se está editando
  ...
  const handleUpdateProduct = async (event) => {
    ...
    // Si es un producto con receta, recalculamos el costo basado en la receta editada
    ...
    try {
      ...
      await updateDoc(productDocRef, {
        ...
        cost: finalCost,
        recipe: editingItem.recipe || [], // Aseguramos que siempre sea un array
      });
  ...
  const handleAddInsumo = async (event) => {
    ...
    try {
      ...
      toast.success(`Insumo añadido! Costo por ${usageUnit}: $${costPerUsageUnit.toFixed(3)}`);
  ...
  const reportData = useMemo(() => {
    ...
    // --- AÑADE ESTA NUEVA LÓGICA DE CÁLCULO ---
    // 1. Agrupamos los ingresos por nombre de producto
    const revenueByProduct = filteredSales.reduce((acc, sale) => {
      const revenue = sale.price * sale.quantity;
      acc[sale.productName] = (acc[sale.productName] || 0) + revenue;
      return acc;
    }, {});
    
    // 2. Preparamos los datos para el gráfico
    const chartLabels = Object.keys(revenueByProduct);
    const chartDataValues = Object.values(revenueByProduct);
      ...
      <div className={styles.tabContent}>
                    ...
                    {filteredInsumos.length === 0 ? (<p>No se encontraron insumos.</p>) : (filteredInsumos.map((insumo) => (
                      <li key={insumo.id} className={`${styles.insumoItem} ${(insumo.alertThreshold != null && insumo.stockInUsageUnit <= insumo.alertThreshold) ? styles.lowStock : ""}`}>
                        <div className={styles.insumoInfo}>
                          ...
                          <span className={styles.insumoCost}>Costo: ${(insumo.costPerUsageUnit || 0).toFixed(3)} / {insumo.usageUnit}</span>
                        </div>
        ...
        {activeTab === "reportes" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
            ...
            {/* --- AÑADE ESTA NUEVA SECCIÓN PARA EL GRÁFICO --- */}
          <section className={styles.card}>
              ...
              {editingItem.itemType === 'insumo' && (
                <form onSubmit={handleUpdateInsumo}>
                  ...
                  <div className={styles.inputGroup}>
                    ...
                    <input type="number" className={styles.input} value={editingItem.costPerUsageUnit} onChange={(e) => setEditingItem({ ...editingItem, costPerUsageUnit: e.target.value })} min="0" step="0.001"/>
                  </div>
                  <div className={styles.inputGroup}>
                ...
                <form onSubmit={handleUpdateProduct}>
                  ...
                  <div className={styles.inputGroup}>
                    ...
                    <input 
                      ...
                      // Deshabilitamos el input si es un producto de reventa, ya que su nombre depende del insumo
                  ...
                  {/* Mostramos la sección de receta solo si el producto es de tipo 'recipe' */}
                  {editingItem.type === 'recipe' && (
                    <>
                      ...
                      <ul className={styles.recipeList}>
                        {editingItem.recipe.length === 0 ? (<li className={styles.recipeEmpty}>Sin ingredientes.</li>) : (editingItem.recipe.map((item, index) => (
