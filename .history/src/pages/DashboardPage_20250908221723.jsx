...
function DashboardPage({ user }) {
  ...
  const handleUpdateProduct = async (event) => {
    ...
    try {
      ...
      await updateDoc(productDocRef, {
        ...
        alertThreshold: Number(editingProduct.alertThreshold),
      });
  ...
  const reportData = useMemo(() => {
    // 1. Filtramos el historial de ventas ANTES de cualquier otro cálculo
    const filteredSales = selectedDate
      ? salesHistory.filter((sale) => {
          // Convertimos la fecha ISO de la venta a formato YYYY-MM-DD para comparar
          const saleDate = new Date(sale.date).toLocaleDateString("en-CA");
          return saleDate === selectedDate;
        })
      : salesHistory; // Si no hay fecha seleccionada, usamos el historial completo

    // 2. AHORA, todos los cálculos siguientes usan 'filteredSales', no 'salesHistory'
    ...
    // Los cálculos de inventario no se ven afectados por el filtro de fecha
    ...
    // 3. Añadimos 'selectedDate' al array de dependencias
  }, [salesHistory, products, selectedDate]);
  // ...
...
<input 
            ...
            <section className={styles.card}>
              ...
              {/* Lógica de filtrado y renderizado único */}
              {(() => {
                  ...
                  <div className={styles.productGrid}>
                    {filteredProducts.length === 0 ? (
                      ...
                      filteredProducts.map((product) => (
                        <div key={product.id} className={`${styles.productCard} ${product.quantity <= (product.alertThreshold || 0) ? styles.lowStock : ''}`}>
        ...
        {activeTab === "reportes" && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            {/* --- AÑADE ESTE NUEVO BLOQUE DE FILTRO --- */}
            <div className={styles.filterControls}>
              ...
              <input
                ...
                id="date-filter"
                className={styles.input}
                value={selectedDate || ""} // Muestra la fecha o un string vacío si es null
                onChange={(e) => setSelectedDate(e.target.value)}
              />
              {/* Este botón solo aparecerá si hay una fecha seleccionada */}
              {selectedDate && (
                <button
            ...
            <section className={styles.card}>
              ...
              <ul className={styles.ticketGroupList}>
                {reportData.groupedSalesArray.length === 0 ? ( // <-- Corrección aquí
              ...
              <form onSubmit={handleUpdateProduct}>
                ...
                <div className={styles.inputGroup}>
                  ...
                  <input 
                    type="number" 
                    className={styles.input} 
                    value={editingProduct.alertThreshold || 0} // Muestra 0 si no está definido
                    onChange={(e) => setEditingProduct({ ...editingProduct, alertThreshold: e.target.value })}
                  />
                </div>
