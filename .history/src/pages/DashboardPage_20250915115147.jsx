...
function DashboardPage({ user }) {
  ...
  const [selectedInsumoIdForRecipe, setSelectedInsumoIdForRecipe] =
    useState("");
  ...
  // Este useEffect se encarga de cargar los datos y escuchar los cambios en tiempo real.
  useEffect(() => {
    if (!user) return;
    ...
    // Carga inicial de todos los datos cuando el componente se monta.
    fetchProducts();
    fetchInsumos();
    fetchSales();

    // Suscripción a los cambios en la base de datos.
    const channel = supabase
      .channel("public-db-changes")
      .on("postgres_changes", { event: "*", schema: "public" }, (payload) => {
        console.log("Cambio detectado en la tabla:", payload.table);

        // --- ESTA LÓGICA ES LA QUE HACE LA MAGIA ---
        // Identifica qué tabla cambió y solo recarga esa parte de los datos.
        if (payload.table === "products") {
          fetchProducts();
        } else if (payload.table === "insumos") {
          // Si el cambio fue en 'insumos', llama a fetchInsumos() para actualizar el estado.
          fetchInsumos();
        } else if (payload.table === "sales") {
          fetchSales();
        }
      })
      .subscribe();
  ...
  const handleAddProduct = async (event) => {
    ...
    if (newProduct.type === "recipe") {
      ...
      if (newProduct.recipe.length === 0) {
        toast.error(
          "Para un producto con receta, debes agregar al menos un insumo."
        );
        return;
      }
      finalCost = newProduct.recipe.reduce(
        (sum, item) => sum + item.quantityUsed * item.cost,
        0
      );
      ...
      if (!newProduct.insumoId) {
        toast.error(
          "Para un producto de reventa, debes seleccionar el insumo correspondiente."
        );
        return;
      }
      // LÍNEA CORREGIDA
      const insumoSource = insumos.find(
        (i) => i.id === Number(newProduct.insumoId)
      );
      if (!insumoSource) {
  ...
  const handleAddRecipeItem = () => {
    if (
      !selectedInsumoIdForRecipe ||
      !recipeQuantity ||
      isNaN(recipeQuantity) ||
      Number(recipeQuantity) <= 0
    ) {
      toast.error("Selecciona un insumo y una cantidad válida.");
      return;
    }
    const insumoDetails = insumos.find(
      (i) => i.id === selectedInsumoIdForRecipe
    );
    if (!insumoDetails) {
  ...
  const handleUpdateProduct = async (event) => {
    ...
    if (editingItem.type === "recipe") {
      finalCost = editingItem.recipe.reduce(
        (sum, item) => sum + item.quantityUsed * item.cost,
        0
      );
    }
  ...
  const handleAddInsumo = async (event) => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success(
        `Insumo añadido! Costo por ${usageUnit}: $${costPerUsageUnit.toFixed(
          2
        )}`
      );
      setNewInsumo({
  ...
  const handleIncrementInsumoQuantity = async (insumoId, currentQuantity) => {
    try {
      const { error } = await supabase
        .from("insumos")
        .update({ stockInUsageUnit: currentQuantity + 1 })
        .eq("id", insumoId);
      if (error) throw error;
    } catch (error) {
  ...
  const handleDeleteInsumo = async (insumoId, insumoName) => {
    if (
      window.confirm(`¿Seguro que quieres eliminar el insumo "${insumoName}"?`)
    ) {
      try {
        const { error } = await supabase
          .from("insumos")
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          alertThreshold: editingItem.alertThreshold
            ? Number(editingItem.alertThreshold)
            : null,
        })
        .eq("id", editingItem.id);
      ...
      const stockUpdatePromises = currentTicket.flatMap((ticketItem) => {
        if (ticketItem.type === "recipe" && ticketItem.recipe) {
          return ticketItem.recipe.map((recipeItem) => {
            const insumoInStock = insumos.find(
              (i) => i.id === recipeItem.insumoId
            );
            const quantityToDeduct =
              recipeItem.quantityUsed * ticketItem.quantity;
            ...
            return supabase
              ...
              .update({
                stockInUsageUnit:
                  insumoInStock.stockInUsageUnit - quantityToDeduct,
              })
              .eq("id", recipeItem.insumoId);
          });
        } else if (ticketItem.type === "direct" && ticketItem.insumoId) {
          const insumoInStock = insumos.find(
            (i) => i.id === ticketItem.insumoId
          );
  ...
  const handleLogout = async () => {
    try {
      // Limpiamos la bandera de bienvenida para que pueda mostrarse en el próximo inicio de sesión.
  ...
  const reportData = useMemo(() => {
    const filteredSales = selectedDate
      ? salesHistory.filter(
          (sale) =>
            new Date(sale.date).toLocaleDateString("en-CA") === selectedDate
        )
      : salesHistory;
    ...
    const chartLabels = Object.values(salesByProductId).map(
      (product) => product.name
    );
    const chartDataValues = Object.values(salesByProductId).map(
      (product) => product.totalRevenue
    );
    ...
    const inventoryValueByCost = insumos.reduce(
      (sum, insumo) =>
        sum + (insumo.costPerUsageUnit || 0) * insumo.stockInUsageUnit,
      0
    );
    return {
      groupedSalesArray,
          ...
          >
            <section className={styles.card}>
              ...
              <form
                ...
                className={styles.productFormGrid}
              >
                <div className={styles.productTypeSelector}>
                  <label>
                    <input
                      type="radio"
                      value="direct"
                      checked={newProduct.type === "direct"}
                      onChange={(e) =>
                        setNewProduct({
                          ...newProduct,
                          type: e.target.value,
                  ...
                  <label>
                    ...
                    />
                    Producido (Usa Receta)
                  </label>
                </div>
                {newProduct.type === "direct" ? (
                  <select
                    value={newProduct.insumoId}
                    onChange={(e) => {
                      ...
                      setNewProduct({
                        ...
                        name: selectedInsumo ? selectedInsumo.name : "",
                      });
                    }}
                    className={styles.input}
                  >
                    <option value="" disabled>
                      -- Selecciona un Insumo para Vender --
                    </option>
                    {insumos.map((insumo) => (
                  ...
                  <input
                    type="text"
                    placeholder="Nombre del Producto con Receta"
                    className={styles.input}
                ...
                {newProduct.type === "recipe" && (
                  <>
                    ...
                    <div className={styles.recipeFormGrid}>
                      <select
                        value={selectedInsumoIdForRecipe}
                        onChange={(e) =>
                          setSelectedInsumoIdForRecipe(e.target.value)
                        }
                        className={styles.input}
                      >
                        <option value="" disabled>
                    ...
                    <ul className={styles.recipeList}>
                      {newProduct.recipe.length === 0 ? (
                        <li className={styles.recipeEmpty}>
                        ...
                        newProduct.recipe.map((item, index) => (
                          <li key={index} className={styles.recipeItem}>
                            <span>
                              {item.quantityUsed} {item.unit} de {item.name}
                            </span>
                            <button
                              type="button"
          ...
          >
            <section className={styles.card}>
              ...
              <form
                ...
                className={styles.insumoFormGrid}
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewInsumo({ ...newInsumo, name: e.target.value })
                  }
                />
                <input
                  type="number"
                  ...
                  className={styles.input}
                  value={newInsumo.purchaseQuantity}
                  onChange={(e) =>
                    setNewInsumo({
                      ...newInsumo,
                ...
                <select
                  className={styles.input}
                  value={newInsumo.purchaseUnit}
                  onChange={(e) =>
                    setNewInsumo({ ...newInsumo, purchaseUnit: e.target.value })
                  }
                >
                ...
                <input
                  ...
                  className={styles.input}
                  value={newInsumo.totalCost}
                  onChange={(e) =>
                    setNewInsumo({ ...newInsumo, totalCost: e.target.value })
                  }
                  min="0"
                />
                <h3 className={styles.formSectionTitle}>
                  2. ¿Cómo lo usas en tus recetas?
                </h3>
                <select
                  className={styles.input}
                  value={newInsumo.usageUnit}
                  onChange={(e) =>
                    setNewInsumo({ ...newInsumo, usageUnit: e.target.value })
                  }
                >
                ...
                <input
                  type="number"
                  placeholder={`# de ${newInsumo.usageUnit} por cada ${newInsumo.purchaseUnit}`}
                  className={styles.input}
                  value={newInsumo.conversionRate}
                  onChange={(e) =>
                    setNewInsumo({
                      ...newInsumo,
                ...
                <input
                  type="number"
                  placeholder={`Alerta de stock (en ${newInsumo.usageUnit})`}
                  className={styles.input}
            ...
            <section className={styles.card}>
              ...
              <div className={styles.filtersContainer}>
                ...
                <select
                  ...
                  onChange={(e) => setFilterByProductId(e.target.value)}
                  className={styles.input}
                >
              ...
              {(() => {
                ...
                if (filterByProductId) {
                  const selectedProduct = products.find(
                    (p) => p.id === filterByProductId
                  );
                  if (selectedProduct && selectedProduct.recipe) {
                    const recipeInsumoIds = selectedProduct.recipe.map(
                      (item) => item.insumoId
                    );
                    filteredInsumos = insumos.filter((insumo) =>
                      recipeInsumoIds.includes(insumo.id)
                    );
                  }
                }
                if (searchTerm) {
                      ...
                      filteredInsumos.map((insumo) => (
                        <li
                          ...
                          className={`${styles.insumoItem} ${
                            insumo.alertThreshold != null &&
                            insumo.stockInUsageUnit <= insumo.alertThreshold
                              ? styles.lowStock
                              : ""
                          }`}
                        >
                          <div className={styles.insumoInfo}>
                            ...
                            <span className={styles.insumoCost}>
                              Costo: $
                              {(insumo.costPerUsageUnit || 0).toFixed(2)} /{" "}
                              {insumo.usageUnit}
                            </span>
                          </div>
                          <div className={styles.insumoDetails}>
                            <div className={styles.actionButtons}>
                              <button
                                onClick={() =>
                                  handleDecrementInsumoQuantity(
                                    insumo.id,
                                    insumo.stockInUsageUnit
                                  )
                                }
                              >
                                -
                            ...
                            <div className={styles.insumoActions}>
                              <button
                                className={styles.actionButton}
                                onClick={() => openEditModal(insumo, "insumo")}
                              >
                                Editar
                              </button>
                              <button
                                className={`${styles.actionButton} ${styles.deleteButton}`}
                                onClick={() =>
                                  handleDeleteInsumo(insumo.id, insumo.name)
                                }
                              >
                                Borrar
                              </button>
              ...
              <div className={styles.chartContainer}>
                {reportData.chartLabels && reportData.chartLabels.length > 0 ? (
                  <SalesChart
                    ...
                    dataValues={reportData.chartDataValues}
                  />
                ) : (
            ...
            >
              {editingItem.itemType === "insumo" && (
                <form onSubmit={handleUpdateInsumo}>
                  ...
                  <div className={styles.inputGroup}>
                    <label>
                      Costo por Unidad de Uso ({editingItem.usageUnit})
                    </label>
                    <input
                      type="number"
                      className={styles.input}
                      value={editingItem.costPerUsageUnit}
                      onChange={(e) =>
                        setEditingItem({
                          ...editingItem,
                          costPerUsageUnit: e.target.value,
                        })
                      }
                      min="0"
                      step="0.01"
                  ...
                  <div className={styles.inputGroup}>
                    <label>
                      Umbral de Alerta de Stock (en {editingItem.usageUnit})
                    </label>
                    <input
                      type="number"
                      className={styles.input}
                      value={editingItem.alertThreshold || ""}
                      onChange={(e) =>
                        setEditingItem({
                          ...editingItem,
                          alertThreshold: e.target.value,
                  ...
                  {editingItem.type === "recipe" && (
                    <>
                      ...
                      <div className={styles.recipeFormGrid}>
                        ...
                        <button
                          type="button"
                          onClick={handleAddRecipeItemToEdit}
                          className={styles.addRecipeButton}
                        >
                          +
                        </button>
                          ...
                          editingItem.recipe.map((item, index) => (
                            <li key={index} className={styles.recipeItem}>
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handleRemoveRecipeItemFromEdit(index)
                                }
                                className={styles.removeRecipeButton}
                              >
                                x
