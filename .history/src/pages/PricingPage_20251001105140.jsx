...
// ⚠️ Ajusta este ID de precio a tu price real de Stripe
const BASIC_PRICE_ID = import.meta.env.VITE_PRICE_ID || "price_1SCmeY25dmLg7iI6N1zTDmK0";
...
function PricingPage() {
  ...
  /** Crea (si hace falta) y retorna el stripe_customer_id */
  const ensureStripeCustomer = async (currentUser, accessToken) => {
    // 1) Revisa si ya lo tienes en tu tabla
    ...
    if (qErr) {
      ...
      // seguimos, intentaremos crearlo si no existe
    }
    if (profile?.stripe_customer_id) return profile.stripe_customer_id;

    // 2) Crear en Stripe vía Edge Function "stripe-customer"
    ...
    const { data, error } = await supabase.functions.invoke("stripe-customer", {
      ...
      // Si tu función necesita datos extra, pásalos aquí:
      // body: { email: currentUser.email }
    });

    if (error) {
      console.error("[stripe-customer invoke error]", error);
      toast.error("No se pudo crear tu perfil de cliente.", { id: toastId });
      throw new Error(error.message || "Edge function failed (stripe-customer)");
    }
