...
function DashboardPage({ user }) {
  ...
  useEffect(() => {
    ...
    // Listener para la colección de INSUMOS
  ...
  const handleAddInsumo = async (event) => {
    ...
    if (
      insumoName.trim() === "" ||
      isNaN(insumoQuantity) || Number(insumoQuantity) <= 0 ||
      insumoPrice.trim() === "" || isNaN(insumoPrice) || Number(insumoPrice) < 0 ||
      insumoCost.trim() === "" || isNaN(insumoCost) || Number(insumoCost) < 0
    ) {
    ...
    try {
      ...
      const newInsumoData = {
        name: insumoName,
        quantity: Number(insumoQuantity),
        price: Number(insumoPrice),
        cost: Number(insumoCost),
        alertThreshold: alertThreshold ? Number(alertThreshold) : null,
      };
      await addDoc(insumosCollectionRef, newInsumoData);
        ...
        {activeTab === "insumos" && (
          ...
          >
            <section className={styles.card}>
              ...
              <form onSubmit={handleAddInsumo} className={styles.formGrid}>
                <input
                  type="text"
                ...
                <input
                  ...
                  className={styles.input}
                  value={insumoPrice}
                  onChange={(e) => setInsumoPrice(e.target.value)}
                  min="0"
                />
                <input
                  type="number"
                        ...
                        >
                          ...
                          <div className={styles.productCardStock}>
                            ...
                            <div className={styles.actionButtons}>
                              <button
                                onClick={() =>
                                  handleDecrementQuantity(insumo.id)
                                }
                              >
                                -
                              </button>
                              ...
                              <button
                                onClick={() =>
                                  handleIncrementQuantity(insumo.id)
                                }
                              >
                                +
                              </button>
