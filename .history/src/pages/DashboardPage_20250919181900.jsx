...
import { useNavigate } from "react-router-dom"; // <-- Asegúrate de tener este import
...
function DashboardPage({ user }) {
  ...
  const [currentView, setCurrentView] = useState('tabs'); // 'tabs' o 'settings'
  ...
  const handleAddRecipeItem = () => {
    ...
    const insumoDetails = insumos.find(
      (i) => i.id === Number(selectedInsumoIdForRecipe)
    );
    if (!insumoDetails) {
  ...
  // Reemplaza la función que empieza en la LÍNEA 573
              ...
              {(() => {
                ...
                if (filterByProductId) {
                  const selectedProduct = products.find(
                    (p) => p.id === Number(filterByProductId)
                  );
