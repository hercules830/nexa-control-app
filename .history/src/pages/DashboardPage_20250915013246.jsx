...
function DashboardPage({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        const { data: productsData, error: productsError } = await supabase
          .from('products')
          .select('*')
          .eq('user_id', user.id); // Solo trae los productos de este usuario
        ...
        const { data: insumosData, error: insumosError } = await supabase
          .from('insumos')
          .select('*')
          .eq('user_id', user.id); // Solo trae los insumos de este usuario
        ...
        const { data: salesData, error: salesError } = await supabase
          .from('sales')
          .select('*')
          .eq('user_id', user.id) // Solo trae las ventas de este usuario
    ...
    // Llama a la función para la carga inicial.
    fetchAllData();

    // Crea UN SOLO canal para escuchar cambios en CUALQUIER tabla.
    ...
    // Función de limpieza para remover el canal.
    return () => {
