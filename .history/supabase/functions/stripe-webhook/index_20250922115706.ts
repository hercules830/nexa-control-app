...
serve(async (req) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // Obtenemos el ID de la suscripción desde el evento de Stripe.
      ...
      // Ahora actualizamos tanto el estado como el ID de la suscripción.
      const updateResponse = await fetch(`${supabaseUrl}/rest/v1/profiles?id=eq.${profile.id}`, {
        ...
        body: JSON.stringify({ 
          ...
          stripe_subscription_id: subscriptionId // <-- Guardamos el nuevo ID
        })
      });
      ...
      console.log(`Suscripción ${subscriptionId} activada para el usuario ${profile.id}`);
    }
  } catch (err) {
