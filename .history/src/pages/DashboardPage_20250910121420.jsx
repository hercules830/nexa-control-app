...
function DashboardPage({ user }) {
  ...
  const [newInsumo, setNewInsumo] = useState({
    name: "",
    purchaseQuantity: "",
    purchaseUnit: "kg",     // Valor por defecto: Kilogramo
    totalCost: "",
    usageUnit: "g",         // Valor por defecto: Gramo
    conversionRate: "",
    alertThreshold: "",
  });
  ...
  // Esta función es un placeholder. La desarrollaremos más adelante.
            ...
            <section className={styles.card}>
              ...
              <form onSubmit={handleAddInsumo} className={styles.insumoFormGrid}>
                ...
                <input
                  ...
                  onChange={(e) => setNewInsumo({ ...newInsumo, purchaseQuantity: e.target.value })}
                  min="0"
                />
                <select
                  className={styles.input}
                  value={newInsumo.purchaseUnit}
                  onChange={(e) => setNewInsumo({ ...newInsumo, purchaseUnit: e.target.value })}
                >
                ...
                <select
                  ...
                  onChange={(e) => setNewInsumo({ ...newInsumo, usageUnit: e.target.value })}
                >
                ...
                <input
                  ...
                  onChange={(e) => setNewInsumo({ ...newInsumo, conversionRate: e.target.value })}
                  min="0"
                />
                <input
                  type="number"
                    ...
                    {filteredInsumos.length === 0 ? (<p>No se encontraron insumos.</p>) : (filteredInsumos.map((insumo) => (
                      <li key={insumo.id} className={`${styles.insumoItem} ${(insumo.alertThreshold != null && insumo.quantity <= insumo.alertThreshold) ? styles.lowStock : ""}`}>
                        <div className={styles.insumoInfo}>
                          ...
                          <span className={styles.insumoCost}>Costo: ${(insumo.cost || 0).toFixed(2)} / {insumo.unit}</span>
                        </div>
