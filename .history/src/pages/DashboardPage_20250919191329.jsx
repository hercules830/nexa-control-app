...
function DashboardPage({ user }) {
  ...
  const handleAddInsumo = async (event) => {
    ...
    try {
      const { error } = await supabase.from("insumos").insert([{ name, purchaseUnit: purchaseUnit, usageUnit: usageUnit, conversionRate: conversionRateNum, costPerUsageUnit: costPerUsageUnit, stockInUsageUnit: totalUsageQuantity, alertThreshold: alertThreshold ? Number(alertThreshold) : null, user_id: user.id }]);
      if (error) throw error;
  ...
  const handleUpdateInsumo = async (event) => {
    ...
    if (!editingItem) return;
    try {
      const { error } = await supabase.from("insumos").update({ name: editingItem.name, costPerUsageUnit: Number(editingItem.costPerUsageUnit), alertThreshold: editingItem.alertThreshold ? Number(editingItem.alertThreshold) : null }).eq("id", editingItem.id);
      if (error) throw error;
  ...
  const handleAddItemToTicket = (event) => {
    ...
    const newItem = { productId: productToAdd.id, name: productToAdd.name, quantity: saleQuantity, price: productToAdd.price, cost: productToAdd.cost || 0, recipe: productToAdd.recipe || [], type: productToAdd.type, insumoId: productToAdd.insumoId };
  ...
  const handleFinalizeSale = async () => {
    ...
    try {
      const salesInsertData = currentTicket.map((ticketItem) => ({ ticketId: Date.now(), productId: ticketItem.productId, productName: ticketItem.name, quantity: ticketItem.quantity, price: ticketItem.price, cost: ticketItem.cost, profit: (ticketItem.price - ticketItem.cost) * ticketItem.quantity, paymentMethod, date: new Date().toISOString(), user_id: user.id }));
  ...
  const reportData = useMemo(() => {
    ...
    const profitByProduct = filteredSales.reduce((acc, sale) => { acc[sale.productName] = (acc[sale.productName] || 0) + sale.profit; return acc; }, {});
