...
function DashboardPage({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        const { data: productsData, error: productsError } = await supabase.from('products').select('*').eq('user_id', user.id);
        if (productsError) throw productsError;
        setProducts(productsData);

        const { data: insumosData, error: insumosError } = await supabase.from('insumos').select('*').eq('user_id', user.id);
        if (insumosError) throw insumosError;
        setInsumos(insumosData);

        const { data: salesData, error: salesError } = await supabase.from('sales').select('*').eq('user_id', user.id).order('ticketId', { ascending: false });
