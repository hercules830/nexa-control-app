...
function DashboardPage({ user }) {
  ...
  const [filterByProductId, setFilterByProductId] = useState("");
    ...
    // REEMPLAZA TU useEffect COMPLETO POR ESTE BLOQUE

  useEffect(() => {
    if (!user) return;

    // 1. Función para obtener todos los datos de una vez.
    const fetchAllData = async () => {
      try {
        ...
        const { data: salesData, error: salesError } = await supabase
          .from('sales').select('*').order('ticketId', { ascending: false });
    ...
    // 2. Llama a la función para la carga inicial.
    fetchAllData();

    // 3. Crea UN SOLO canal para escuchar cambios en CUALQUIER tabla pública.
    const channel = supabase.channel('public-tables');
    
    channel
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload) => {
        console.log('Cambio detectado en la base de datos, recargando datos...', payload);
        // Cuando algo cambia, vuelve a cargar todos los datos.
        fetchAllData();
      })
      .subscribe();

    // 4. Función de limpieza para remover el único canal.
    return () => {
  ...
  const handleFinalizeSale = async () => {
    ...
    try {
      // Supabase no tiene un "batch" igual a Firestore para el cliente,
      // ejecutaremos las promesas en paralelo.
        ...
        if (ticketItem.type === 'recipe') {
            for (const recipeItem of ticketItem.recipe) {
                ...
                stockUpdatePromises.push(
                    supabase
                        .from('insumos')
                        .update({ stockInUsageUnit: insumoInStock.stockInUsageUnit - quantityToDeduct })
      ...
      const salesInsertPromise = supabase.from('sales').insert(salesInsertData);
      
      // Ejecutamos todas las operaciones
      const results = await Promise.all([salesInsertPromise, ...stockUpdatePromises]);

      // Verificamos si alguna operación falló
  ...
  const handleLogout = async () => {
    try {
      ...
      if (error) throw error;
      // No necesitamos hacer nada más. El listener en App.jsx detectará
      // el cambio y nos llevará automáticamente a la página de bienvenida/login.
    } catch (error) {
