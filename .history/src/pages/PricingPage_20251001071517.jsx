...
function PricingPage() {
  ...
  const ensureStripeCustomer = async (currentUser, accessToken) => {
    ...
    if (qErr) {
      ...
      // no hacemos throw, la función podría crearlo igual
    }
    if (profile?.stripe_customer_id) return profile.stripe_customer_id;

    // 2) Crear en Stripe vía Edge Function
    ...
    // si Supabase marca “non-2xx”, ‘error’ viene con info básica,
    // pero ‘data’ puede traer { error: "..."} si la función lo devolvió
    if (error) {
      console.error("[invoke error]", error);
      // intenta mostrar mensaje del body si vino
      if (data?.error) throw new Error(data.error);
      throw new Error(error.message || "Edge function failed");
    }
    
    if (!data?.checkoutUrl) throw new Error("No checkoutUrl from function");
