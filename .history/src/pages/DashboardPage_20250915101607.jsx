...
function DashboardPage({ user }) {
  ...
  // --- CORRECCIÓN CLAVE: Lógica de carga de datos y suscripción a cambios ---
  useEffect(() => {
    if (!user) return;
    ...
    const channel = supabase.channel('public-db-changes')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload) => {
        ...
        // Identifica qué tabla cambió y solo actualiza esa parte del estado
        if (payload.table === 'products') {
          fetchProducts();
        } else if (payload.table === 'insumos') {
          fetchInsumos(); // <- Esto asegura que los insumos se actualicen automáticamente
